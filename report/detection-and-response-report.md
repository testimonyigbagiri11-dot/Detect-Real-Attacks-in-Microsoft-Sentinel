...
|---|---:|---|
| `10.20.14.9` | 30 | Highest-volume source; internal address |
| `203.0.113.77` | 14 | External source targeting `opsadmin` |
| `198.51.100.23` | 11 | External source targeting multiple accounts |
...
|---|---|---|
| `T1110` | Brute Force | External IP `203.0.113.77` generated repeated failed SSH authentication attempts against the `opsadmin` account before successfully authenticating. |
| `T1110.003` | Brute Force: Password Spraying | External IP `198.51.100.23` attempted authentication against 11 different usernames, with approximately one failed attempt per account. |
| `T1078` | Valid Accounts | The attacker successfully authenticated as `opsadmin` after the repeated failed login attempts. |
| `T1003.008` | OS Credential Dumping: `/etc/passwd` and `/etc/shadow` | Following the successful login, the compromised account used `sudo` to execute `/usr/bin/cat /etc/shadow`, indicating an attempt to access stored Linux password hashes. |
